...
7a. **State.json metric verification** — After validation, verify that state.json aggregate metrics match actual test output. Update `package.commit` to the current package version. Update `last_tested` and `package_commit` for all covered types to the current date and commit. Verify unit test count (383), E2E pass count (73/73), and TS parity count (73/73) match the runner output — if they differ, update state.json immediately. This prevents metric drift across cycles. (Per [audit #78](https://github.com/EvaLok/schema-org-json-ld-audit/issues/78).)
